...
class BackgroundGenerator(object):
    ...
    def setFValues(self, input_frame, generated_bg, T=80):
        ...
        F1 = np.sum(bg)/self.getTotalPixal()
        # normalize F1
    ...
    def returnValues(self):
        if self.debug:
            return self.image_a, self.image_b, self.image_c, self.image_d, self.image_diff, self.combined_diff_img
        else:
    ...
    def apply(self, frame):
        ...
        if (self.update_image_a_count >= self.update_image_a_rate):
            ...
            self.setFValues(frame, self.image_d)
            # get optimal threshold
            self.p = self.fuzzy.get_threshold(self.f1, self.f2) 
            return self.returnValues()
        # generate difference image based on the optimal threshold
        image_diff = self.getBinaryDiffImage(frame, self.image_d, self.p) # same ops for imgb
        # Morphorlogical Operation with Connected Components Labeling
        stats, combined_diff_img = self.humanRegionConfirmation(image_diff)
...
if __name__ == '__main__':
    ...
    for i in xrange(1000):
        ...
        ret,frame = cap.read()
        median, binary, ccl, bg, diff, combined_diff_img = bgg.apply(frame)

        
        im1.set_data(median)
        im2.set_data(binary)
        im3.set_data(diff)
        im4.set_data(combined_diff_img)
